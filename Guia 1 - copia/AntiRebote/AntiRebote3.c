...
void MaquinaEstado(void){
    ...
    switch (estado_actual) { //entro en el switch
        ...
        case SUELTO:
            if(boton == 1){// botón presionado (activo en 0)
            ...
            // El botón se liberó antes de tiempo 
            estado_actual = SUELTO;
         }
            break;
        ...
        case Q_PRES:
            if(boton == 0){
                // pasó el tiempo de rebote y sigue apretado
                estado_actual = Q_SUELTO;
            } 
            break;

        case Q_SUELTO:// estuvo presionado y ahora se soltó estable
